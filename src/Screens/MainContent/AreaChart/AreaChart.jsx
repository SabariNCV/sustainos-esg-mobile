...
export default function AreaChart(props) {
  ...
  let areaChartDataIs = basicDetails
  if (page === 'analytics') {
    areaChartDataIs = basicDetails
  } else {
    areaChartDataIs = checkTheCond[chartId]["areaChart-colors"]
  }
  const [areaChartData, setAreaChartData] = useState(areaChartDataIs)
  ...
  useEffect(() => {
    ...
    fetchDataAndRender()
    let refreshTime = 0;
    const refreshFreq = areaChartData?.refreshFreq?.split(" ")
  ...
  const fetchDataAndRender = async () => {
    ...
    if (page === 'analytics') {
      ...
      if (type === "panel") {
        ...
        paramData = areaChartData
        if (areaChartData?.aggregateTime === "custom") {
          fromDate = areaChartData?.fromDate
          toDate = areaChartData?.toDate
        } else {
          const timeIs = TimingsConversion(areaChartData?.aggregateTime)
          fromDate = timeIs[0]
          toDate = timeIs[1]
        }
      }
    ...
    if (paramData?.aggregateTime === "custom" ||
      ...
      url = `${BASE_URL}dataservice_app/api/parameter_values/?id=${parametersId}&from_date=${fromDate}&to_date=${toDate}&data_type=area&filter_condition=${JSON?.stringify(RequestBody)}`;
    } else {
      url = `${BASE_URL}dataservice_app/api/parameter_values/?id=${parametersId}&data_type=area&time_frequency=${paramData?.aggregateTime}&filter_condition=${JSON?.stringify(RequestBody)}`;
    }
    try {
      ...
      if (data) {
        ...
        parametersId?.forEach((key, ind) => {
          const trace = {
            ...
            fill: 'tozeroy',
            type: 'scatter',
            hoverinfo: areaChartData?.toolTip ? 'all' : 'none',
            yaxis: 'y',
            line: {
              dash: 'solid',
              width: 1.5,
            ...
            yaxis: areaChartData?.multiaxis ? `y${ind + 1}` : 'y'
  ...
  const layout = {
    ...
    annotations: noData ? [{
      ...
      showarrow: false,
      font: {
        family: areaChartData?.fFamily,
        size: 10,
        color: 'red',
      },
    }] : [],
    ...(areaChartData?.multiaxis ? (() => {
      ...
      const yAxisConfigs = Object?.fromEntries(
        areaChartData?.parameters?.map((parameter, index) => {
          let position, isLeft
          if (page === 'analytics' || parmMultiAxis) {
          ...
          return [
            `yaxis${index + 1}`,
            {
              ...
              showline: true,
              zeroline: false,
              showgrid: areaChartData?.isGridPresent && index === 0
            },
          ];
        })
      );
      ...
      return {
        ...
        xaxis: {
          ...
          autorange: true,
          showgrid: areaChartData?.isGridPresent,
          gridcolor: areaChartData?.xGridColor,
          gridwidth: 1,
          griddash: "dot",
          gridshape: "linear",
          showline: true,
          tickfont: {
            family: areaChartData?.xfFamily,
            size: 10,
            color: areaChartData?.xFontColor,
            weight: areaChartData?.xBold ? "bold" : "normal",
          },
          tickangle: 0,
        },
      };
      ...
      xaxis: {
        ...
        showgrid: areaChartData?.isGridPresent,
        nticks: 6,
        //zeroline: !areaChartData?.isGridPresent,
        gridcolor: areaChartData?.xGridColor,
        gridwidth: 1,
        griddash: 'dot',
        gridshape: 'linear',
      ...
      yaxis: {
        autorange: true,
        showgrid: areaChartData?.isGridPresent,
        gridcolor: areaChartData?.yGridColor,
        gridwidth: 1,
        griddash: 'dot',
        gridshape: 'linear',
        showline: true,
        tickfont: {
          family: areaChartData?.yfFamily,
          size: 10,
          color: areaChartData?.yFontColor,
          weight: areaChartData?.yBold ? "bold" : "normal"
        }
      },
    }),
    showlegend: areaChartData?.showLegend,
    legend: {
      y: areaChartData?.legend?.y + 0.2,
      x: areaChartData?.legend?.x,
      orientation: 'h',
      xanchor: 'center',
      traceorder: 'normal',
      itemsizing: 'trace',
      font: {
        family: areaChartData?.legendFamily,
        size: 10,
        weight: 'lighter',
      }
    },
    images: [{
      source: areaChartData?.plotAreaBg,
      xref: "paper",
      yref: "paper",
      x: 0,
      y: 1,
    ...
    paper_bgcolor: areaChartData?.areaBackground,
    plot_bgcolor: areaChartData?.plotAreaBg,
    shapes: [{
      type: 'rect',
      x0: 0,
      x1: 1.0,
      ...
      yref: 'paper',
      line: {
        color: areaChartData?.plotAreaOutline,
        width: 1,
      },
    }],
    chartOutline: `1px solid ${areaChartData?.areaOutline}`,
    chartTitleIs: areaChartData?.chartTitle,
    fontSize: areaChartData?.fSize,
    fontFamily: areaChartData?.fFamily,
    isBold: areaChartData?.isBold,
    isItalic: areaChartData?.isItalic,
    isUnderLine: areaChartData?.isUnderLine,
    isCaseChange: areaChartData?.isCaseChange,
    fontColor: areaChartData?.fontColor,
    fontBgColor: areaChartData?.fontBgColor,
    rndproperties: areaChartData?.reSizeProperties,
  };
